...
function refresh(append) {
    ...
    .then(result => {
        ...
        //status progress
        document.getElementById('progress-bar').style.width=dataAPI.status.progress+"%"
        ...
        document.getElementById('tahun_terdaftar').value = dataAPI.status.periode

        //reset tabel
        $("#table-responsive").empty();
        let tag ='<table class="table-hover table display nowrap" id="tbl-preview" style="width: 100%"><thead><tr id="tbl-header"></tr></thead><tbody id="tbl-body"></tbody></table>'
        ...
        for (let index = 0; index < dataAPI.candidates.length; index++) {
            ...
            tag = '<tr>'
            tag += '<td>'+(index+1)+'</td>' 
            tag += '<td>'+dataAPI['candidates'][index]['periode']+'</td>' 
            dataAPI.criteria.forEach(element => {
                if (dataAPI['candidates'][index][element] != '') {
                    tag += '<td>'+dataAPI['candidates'][index][element]+'</td>' 
                } else {
                    tag += '<td>-</td>' 
                }
            });
