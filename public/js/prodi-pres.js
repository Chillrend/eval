...
function refresh(append) {
  ...
  fetch(url+append, requestOptions)
    ...
    .then(result => {
      ...
      console.log(dataAPI);
      if (datatable != 0) {
        $('#table-prodi-pres').dataTable().fnClearTable();
        $('#table-prodi-pres').dataTable().fnAddData(dataAPI.prodi);
      } else {
        datatable ++      

        $("#table-prodi-pres").DataTable({
          data: dataAPI.prodi,
          responsive: true,
          pageLength: 10,
          autoWidth: false,
  ...
  function editBtn(id) {
    document.getElementById('prodi-prestasi-edit').removeAttribute('hidden');
    document.getElementById('prodi-prestasi-edit').focus();
